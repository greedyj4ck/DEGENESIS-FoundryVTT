// CLASS FOR CREATING ROLL DIALOG FOR PHENOMENON ACTIVATION
export default class RollPhenomenonDialog extends Dialog {
  _onValueChange() {
    // GRAB ALL THE CUSTOM MODIFIERS
    let modifiers = this._getSelectedModifiers();

    // CALCULATE TOTAL MODIFIERS FOR ROLL
    this.data.dialogData.totalRollModifiers.diceModifier =
      this.userEntry.diceModifier +
      this.data.dialogData.prefilled.diceModifier +
      modifiers.diceModifier -
      this.userEntry.overload;
    this.data.dialogData.totalRollModifiers.successModifier =
      this.userEntry.successModifier +
      this.data.dialogData.prefilled.successModifier +
      modifiers.successModifier;
    this.data.dialogData.totalRollModifiers.triggerModifier =
      this.userEntry.triggerModifier +
      this.data.dialogData.prefilled.triggerModifier +
      modifiers.triggerModifier;

    // UPDATE DISPLAYED TOTAL MODIFIERS
    this.TotalDiceModifierDiv[0].innerHTML =
      String(this.data.dialogData.totalRollModifiers.diceModifier) +
      game.i18n.localize("UI.D");
    this.TotalSuccessModifierDiv[0].innerHTML =
      String(this.data.dialogData.totalRollModifiers.successModifier) +
      game.i18n.localize("UI.S");
    this.TotalTriggerModifierDiv[0].innerHTML =
      String(this.data.dialogData.totalRollModifiers.triggerModifier) +
      game.i18n.localize("UI.T");
  }
  ...
  activateListeners(html) {
    ...
    this.TotalSuccessModifierDiv = html.find(
      "div[id='TotalRollSuccessModifier']"
    );
    this.TotalTriggerModifierDiv = html.find(
      "div[id='TotalRollTriggerModifier']"
    );

    // ACTIVATE LISTENERS FOR INPUT FIELDS

    this.overloadInput = html.find("input[name='overload']").change((ev) => {
      this.userEntry.overload = Number(ev.target.value);
      this._onValueChange();
    });

    this.diceModifierInput = html
      .find("input[name='diceModifier']")
      .change((ev) => {
        this.userEntry.diceModifier = Number(ev.target.value);
        this._onValueChange();
      });
    this.successModifierInput = html
      .find("input[name='successModifier']")
      .change((ev) => {
        this.userEntry.successModifier = Number(ev.target.value);
        this._onValueChange();
      });
    this.triggerModifierInput = html
      .find("input[name='triggerModifier']")
      .change((ev) => {
    ...
    this.resetButton = html
      .find(".custom-modifiers-reset-button")
      .click((ev) => {
        this.resetValues();
      });
  }
